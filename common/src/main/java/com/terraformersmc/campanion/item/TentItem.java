...
public class TentItem extends PlaceableTentItem {
	...
	public TentItem(Properties settings, String structureName) {
		super(settings);
		this.structure = ResourceLocation.fromNamespaceAndPath(Campanion.MOD_ID, "tents/" + structureName + "_tent");
	}
	...
	public static void initNbt(ItemStack stack, StructureTemplate structure) {
		...
		CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> {
			tag.put("Blocks", list);
			tag.put("TentSize", NbtUtils.writeBlockPos(new BlockPos(size)));
		});
	}
}
