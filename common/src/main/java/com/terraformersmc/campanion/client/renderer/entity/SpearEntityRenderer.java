...
public class SpearEntityRenderer extends EntityRenderer<SpearEntity> {
	...
	public void render(SpearEntity spear, float f, float g, PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int i) {
		...
		VertexConsumer vertexConsumer = ItemRenderer.getFoilBufferDirect(vertexConsumerProvider, model.renderType(this.getTextureLocation(spear)), false, spear.isEnchanted());
		model.renderToBuffer(matrixStack, vertexConsumer, i, OverlayTexture.NO_OVERLAY, -1);
	...
	public static ResourceLocation getTexture(EntityType<?> type) {
		if (!TEXTURES.containsKey(type)) {
			TEXTURES.put(type, ResourceLocation.fromNamespaceAndPath(Campanion.MOD_ID, "textures/entity/spear/" + EntityType.getKey(type).getPath() + ".png"));
		}
