...
public enum SleepingBagFeatureRenderer {
	...
	private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Campanion.MOD_ID, "textures/entity/sleeping_bag.png");
	...
	public void render(AbstractClientPlayer entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumerProvider, int light, int color) {
		...
		if(entity != camera.getEntity() || camera.isDetached()) {
			...
			if (renderLayer != null) {
				...
				this.bipedModel.renderToBuffer(matrices, vertexConsumer, light, OverlayTexture.NO_OVERLAY, isInvisibleToPlayer ? 654311423 : -1);
			}
		}
		...
		this.model.renderToBuffer(matrices, buffer, light, OverlayTexture.NO_OVERLAY, color);
	}

	protected RenderType getRenderLayer(AbstractClientPlayer entity, boolean showBody, boolean translucent) {
		ResourceLocation identifier = entity.getSkin().texture();
		if (translucent) {
