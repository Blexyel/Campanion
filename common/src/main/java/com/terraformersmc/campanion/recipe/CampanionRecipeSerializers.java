...
public class CampanionRecipeSerializers {
	...
	public static final RecipeSerializer<TentBuildingRecipe> TENT_BUILDING_RECIPE = add("tent_building_recipe", new SimpleCraftingRecipeSerializer<>(TentBuildingRecipe::new));
	public static final RecipeSerializer<TarpRecipe> TARP_RECIPE = add("tarp_recipe", new SimpleCraftingRecipeSerializer<>(TarpRecipe::new));

    private static <S extends RecipeSerializer<T>, T extends Recipe<?>> S add(String id, S serializer) {
		RECIPE_SERIALIZERS.put(ResourceLocation.fromNamespaceAndPath(Campanion.MOD_ID, id), serializer);
        return serializer;
    }
