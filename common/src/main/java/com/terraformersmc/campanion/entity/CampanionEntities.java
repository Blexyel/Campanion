...
public class CampanionEntities {
	...
	public static final EntityType<LawnChairEntity> LAWN_CHAIR = add("lawn_chair", EntityType.Builder.<LawnChairEntity>of((type, world) -> new LawnChairEntity(world), MobCategory.MISC).passengerAttachments((float) (0.25D + (Player.DEFAULT_VEHICLE_ATTACHMENT.y() + LEGACY_PLAYER_RIDING_OFFSET))));
...
//	}
	...
	private static <T extends Entity> EntityType<T> add(String name, EntityType.Builder<T> builder) {
		ResourceLocation id = ResourceLocation.fromNamespaceAndPath(Campanion.MOD_ID, name);
