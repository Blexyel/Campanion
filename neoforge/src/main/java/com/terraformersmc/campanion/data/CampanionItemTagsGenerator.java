...
public class CampanionItemTagsGenerator extends ItemTagsProvider {
	public CampanionItemTagsGenerator(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> provider, CompletableFuture<TagsProvider.TagLookup<Block>> blockTagProvider, ExistingFileHelper existingFileHelper) {
		super(packOutput, provider, blockTagProvider, Campanion.MOD_ID, existingFileHelper);
	}
	...
	protected void addTags(HolderLookup.Provider provider) {
		...
		this.tag(ItemTags.DYEABLE).add(CampanionItems.SLEEPING_BAG);
		this.tag(ItemTags.BREAKS_DECORATED_POTS).addTag(CampanionItemTags.SPEARS);
		this.tag(ItemTags.TRIDENT_ENCHANTABLE).addTag(CampanionItemTags.SPEARS);
		this.tag(ItemTags.DURABILITY_ENCHANTABLE).addTag(CampanionItemTags.SPEARS);
